import React, { useState, useEffect, useCallback } from 'react';
    ...
    const CertificateSystem = ({ trainingId, trainingName }) => {
      ...
      const handleDownloadCertificate = (participant) => {
        const certificate = certificates.find(c => c.participant_id === participant.participant.id);
        if (!certificate) {
        ...
        const reportData = {
          title: `Sertifika - ${participant.participant.employee.first_name} ${participant.participant.employee.last_name}`,
          certificateData: {
            participantName: `${participant.participant.employee.first_name} ${participant.participant.employee.last_name}`,
      ...
      const participantsWithCerts = successfulParticipants.map(p => ({
        ...p,
          ...
          <CardContent>
            {participantsWithCerts.length === 0 ? (
              <div className="text-center py-12 text-gray-500">
                ...
                <p className="mt-2 text-lg">Henüz sertifika oluşturulacak başarılı personel bulunmuyor.</p>
              </div>
            ) : (
                ...
                <table className="w-full">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {participantsWithCerts.map(p => (
                      <tr key={p.id}>
                        <td className="px-4 py-2 whitespace-nowrap text-sm font-medium">{p.participant.employee.first_name} {p.participant.employee.last_name}</td>
