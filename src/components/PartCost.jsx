...
const PartCost = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}><DialogContent className="max-w-5xl bg-white rounded-xl shadow-2xl p-0 overflow-hidden border-0"><DialogHeader className="bg-gradient-to-r from-blue-600 to-blue-500 p-6 text-white"><div className="flex items-center gap-3"><div className="p-2 bg-white/20 rounded-lg">{editingRecord ? <Edit className="h-5 w-5" /> : <Plus className="h-5 w-5" />}</div><div><DialogTitle className="text-xl font-bold text-white">{editingRecord ? 'Günlük Kaydı Düzenle' : 'Yeni Günlük Kayıt'}</DialogTitle><DialogDescription className="text-blue-100 mt-1">Üretim ve maliyet verilerini girin</DialogDescription></div></div></DialogHeader><div className="p-6"><DailyRecordForm onSave={handleSave} productionLines={productionLines} initialData={editingRecord} /></div></DialogContent></Dialog>

      <Dialog open={!!viewingRecord} onOpenChange={() => setViewingRecord(null)}>
        <DialogContent className="max-w-6xl bg-white rounded-xl shadow-2xl p-0 overflow-hidden border-0">
          {viewingRecord && (
            <>
              <DialogHeader className="bg-gradient-to-r from-slate-700 to-slate-600 p-6 text-white">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-white/20 rounded-lg"><Eye className="h-5 w-5" /></div>
                  <div>
                    <DialogTitle className="text-xl font-bold text-white">{new Date(viewingRecord.production_date).toLocaleDateString('tr-TR', { dateStyle: 'full' })} - Üretim Detayları</DialogTitle>
                    <DialogDescription className="text-slate-200 mt-1">Günlük üretim ve maliyet analizi</DialogDescription>
                  </div>
                </div>
              </DialogHeader>
              ...
              <div className="px-6 pb-6 max-h-[40vh] overflow-y-auto border rounded-lg mx-6">
        ...
        <DialogContent className="sm:max-w-md bg-white rounded-xl shadow-2xl border-0 overflow-hidden">
          <DialogHeader className="bg-red-50 p-6 border-b border-red-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="text-lg font-bold text-red-900">Kaydı Silmeyi Onayla</DialogTitle>
                <DialogDescription className="text-red-700 mt-1">{recordToDelete && `${new Date(recordToDelete.production_date).toLocaleDateString('tr-TR')} tarihli günlük kaydı kalıcı olarak silmek istediğinizden emin misiniz? Bu işlem o güne ait tüm parça girişlerini silecektir ve geri alınamaz.`}</DialogDescription>
              </div>
            </div>
          </DialogHeader>
          <div className="p-6">
          ...
          <DialogFooter className="p-6 bg-gray-50 flex items-center justify-end gap-3 border-t border-gray-100">
            <DialogClose asChild><Button variant="outline" className="rounded-lg">İptal</Button></DialogClose>
