...
import { Factory, Plus, Clock, Trash2, Download, Save } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <Card>
          <CardHeader><div className="flex justify-between items-center"><div><CardTitle className="flex items-center space-x-2"><Factory className="h-5 w-5" /><span>Üretim Kaydı & İzlenebilirlik</span></CardTitle><CardDescription>Üretim kayıtlarını takip edin ve parça bazlı izlenebilirlik sağlayın</CardDescription></div><div className="flex space-x-2"><Dialog open={showAddDialog} onOpenChange={setShowAddDialog}><DialogTrigger asChild><Button><Plus className="h-4 w-4 mr-2" />Yeni Kayıt</Button></DialogTrigger><DialogContent className="max-w-2xl bg-white rounded-xl shadow-2xl p-0 overflow-hidden border-0"><DialogHeader className="bg-gradient-to-r from-teal-600 to-teal-500 p-6 text-white"><DialogTitle className="text-2xl font-bold flex items-center gap-2"><Plus className="h-6 w-6 text-white/80" />Yeni Üretim Kaydı</DialogTitle><DialogDescription className="text-teal-100 opacity-90">Üretim kayıt bilgilerini aşağıdan girebilirsiniz.</DialogDescription></DialogHeader><div className="p-6 grid grid-cols-2 gap-5">
          ...
          </div><DialogFooter className="p-6 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3"><Button variant="outline" onClick={() => setShowAddDialog(false)} className="rounded-lg border-gray-300 hover:bg-gray-100 hover:text-gray-900 transition-colors px-6">İptal</Button><Button onClick={handleAddRecord} className="bg-teal-600 hover:bg-teal-700 text-white rounded-lg shadow-md shadow-teal-200 transition-all px-6"><Save className="mr-2 h-4 w-4" />Kaydet</Button></DialogFooter></DialogContent></Dialog><Button variant="outline" onClick={handleGenerateDetailedReport}><Download className="h-4 w-4 mr-2" />Detaylı Rapor</Button></div></div></CardHeader>
          <CardContent>
