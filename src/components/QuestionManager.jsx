    ...
    <Card>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="sm:max-w-[600px] bg-white rounded-xl shadow-2xl p-0 overflow-hidden border-0">
          <DialogHeader className="bg-gradient-to-r from-orange-600 to-orange-500 p-6 text-white">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-white/20 rounded-lg"><FileText className="h-5 w-5" /></div>
              <div>
                <DialogTitle className="text-xl font-bold text-white">{editingQuestion ? 'Soruyu Düzenle' : 'Yeni Soru Ekle'}</DialogTitle>
                <DialogDescription className="text-orange-100 mt-1">Eğitim sınavı için soru bilgilerini girin</DialogDescription>
              </div>
            </div>
          </DialogHeader>
          <div className="p-6 space-y-5 modal-body-scroll">
            <div><Label className="font-medium text-gray-700">Soru Metni</Label><Input className="mt-1.5 rounded-lg border-gray-300 focus:border-orange-500 focus:ring-orange-500" value={formState.question_text} onChange={e => setFormState({ ...formState, question_text: e.target.value })} /></div>
            <div className="grid grid-cols-2 gap-4">
              <div><Label className="font-medium text-gray-700">Soru Tipi</Label><Select value={formState.question_type} onValueChange={type => setFormState({ ...formState, question_type: type })}><SelectTrigger className="mt-1.5 rounded-lg border-gray-300"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="çoktan seçmeli">Çoktan Seçmeli</SelectItem><SelectItem value="doğru/yanlış">Doğru/Yanlış</SelectItem><SelectItem value="açık uçlu">Açık Uçlu</SelectItem></SelectContent></Select></div>
              <div><Label className="font-medium text-gray-700">Puan</Label><Input className="mt-1.5 rounded-lg border-gray-300 focus:border-orange-500 focus:ring-orange-500" type="number" value={formState.points} onChange={e => setFormState({ ...formState, points: parseInt(e.target.value) || 0 })} /></div>
            </div>
            {formState.question_type === 'çoktan seçmeli' && (
              <div>
                <Label className="font-medium text-gray-700">Seçenekler (Doğru olanı işaretleyin)</Label>
                <div className="space-y-2 mt-2">
                  {formState.options.map((opt, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <input type="radio" name="correct_option" checked={opt.is_correct} onChange={() => handleCorrectOptionChange(i)} className="text-orange-600 focus:ring-orange-500" />
                      <Input className="rounded-lg border-gray-300 focus:border-orange-500 focus:ring-orange-500" value={opt.text} onChange={e => handleOptionChange(i, e.target.value)} placeholder={`Seçenek ${i + 1}`} />
                    </div>
                  ))}
                </div>
              </div>
          ...
          <DialogFooter className="p-6 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3">
            <Button variant="outline" className="rounded-lg" onClick={() => setShowForm(false)}>İptal</Button>
            <Button className="rounded-lg bg-orange-600 hover:bg-orange-700" onClick={handleSave}><Save className="mr-2 h-4 w-4" />Kaydet</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </Card>
