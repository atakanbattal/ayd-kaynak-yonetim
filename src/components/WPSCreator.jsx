    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <DialogContent className="max-w-6xl bg-white rounded-xl shadow-2xl p-0 overflow-hidden border-0">
          <DialogHeader className="bg-gradient-to-r from-indigo-600 to-indigo-500 p-6 text-white">
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="text-xl font-bold text-white">{formData.id ? 'WPS Düzenle' : 'Yeni WPS Oluştur'}</DialogTitle>
                <DialogDescription className="text-indigo-100 mt-1">Standartlara dayalı, akıllı öneriler ile WPS oluşturun.</DialogDescription>
              </div>
            </div>
          </DialogHeader>
          ...
          <DialogFooter className="p-6 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3">
            ...
            <Button className="rounded-lg bg-indigo-600 hover:bg-indigo-700" onClick={handleSave}><Save className="h-4 w-4 mr-2" />{formData.id ? 'Güncelle' : 'Kaydet'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showViewDialog} onOpenChange={setShowViewDialog}>
        <DialogContent className="max-w-2xl bg-white rounded-xl shadow-2xl p-0 overflow-hidden border-0">
          <DialogHeader className="bg-gradient-to-r from-slate-700 to-slate-600 p-6 text-white">{selectedWPS && <>
            <div className="flex items-center gap-3">
              <div className="p-2 bg-white/20 rounded-lg"><FileText className="h-5 w-5" /></div>
              <div>
                <DialogTitle className="text-xl font-bold text-white">{selectedWPS.wps_code}</DialogTitle>
                <DialogDescription className="text-slate-200 mt-1">Oluşturulma: {new Date(selectedWPS.created_at).toLocaleString('tr-TR')}</DialogDescription>
              </div>
            </div>
          </>}</DialogHeader>
          <div className="flex-1 overflow-y-auto px-6 py-4 modal-body-scroll">
        ...
        <DialogContent className="sm:max-w-md bg-white rounded-xl shadow-2xl border-0 overflow-hidden">
          <DialogHeader className="bg-red-50 p-6 border-b border-red-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="text-lg font-bold text-red-900">WPS Silmeyi Onayla</DialogTitle>
                <DialogDescription className="text-red-700 mt-1">"{selectedWPS?.wps_code}" kodlu WPS'i kalıcı olarak silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.</DialogDescription>
              </div>
            </div>
          </DialogHeader>
          <div className="p-6">
          ...
          <DialogFooter className="p-6 bg-gray-50 flex items-center justify-end gap-3 border-t border-gray-100">
            <Button variant="outline" className="rounded-lg" onClick={() => setShowDeleteDialog(false)}>İptal</Button>
